...
{
	options=("-l" "-l@" "-le" "-l@e" "-ln" "-Rl" "-lF" "-la" "-lt" "-ltu" "-ltU" "-ltuU" "-ltUu" "-luU" "-lUu" "-lrtU" "-lrtuU" "-lrtUu" "-lruU" "-lrUu" "-lS" "-lrS" "-lSt" "-lStU" "-ltS" "-ltUS" "-lR" "-lRt" "-lRS" "-lF" "-lSF" "-lSFtUr")
	...
	do
		...
		printf "ls %s %s\n\n" $opt $1 > $DIFF_DIR/$testa
		printf "ls %s %s\n\n" $opt $1 > $DIFF_DIR/$testb
		ls $opt $1 | awk '{print $1,$2,$3,$4,$5,$9}' >> $DIFF_DIR/$testa 2> /dev/null
		./ft_ls $opt $1 | awk '{print $1,$2,$3,$4,$5,$9}' >> $DIFF_DIR/$testb 2> /dev/null
		diff $DIFF_DIR/$testa $DIFF_DIR/$testb > $DIFF_DIR/$diffname
		if (( $? == 0 )) ; then
			(( nb_passed++ ))
			mv $DIFF_DIR/$diffname $DIFF_DIR/$diffname"_OK"
